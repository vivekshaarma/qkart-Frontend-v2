...
router.post('/', verifyAuth, async (req, res) => {
    ...
    products.findOne({ _id: req.body.productId }, async (err, product) => {
        ...
        index = await req.user.cart.findIndex(
        ...
        users.update(
            ...
            (err) => {
                ...
                console.log(`User ${req.user.username}'s cart updated to`, req.user.cart);
...
router.post('/checkout', verifyAuth, async (req, res) => {
    ...
    let total = 0;
    for (element of req.user.cart) {
        try {
            product = await getProduct(element.productId);
            if (product == null) {
    ...
    addressIndex = await req.user.addresses.findIndex(
